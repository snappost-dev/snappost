/**
 * Editor.js → HTML (kiracı D1 `content` yedek yolu).
 * Snappost: `templates/dashboard/src/lib/editor.ts` ile aynı mantıkta tutun.
 */
...
export function renderEditorJSToHTML(json: any): string {
  ...
  return json.blocks
    .map((block: any) => {
      switch (block.type) {
        ...
        case 'quote':
          return `<blockquote style="border-left:4px solid #cbd5e1;padding:0.75rem 1rem;margin:1rem 0;color:#475569;font-style:italic">${escapeHtml(block.data.text || '')}${
            block.data.caption
              ? `<cite style="display:block;font-size:0.875rem;margin-top:0.5rem">— ${escapeHtml(block.data.caption)}</cite>`
              : ''
          }</blockquote>`;
        case 'code':
