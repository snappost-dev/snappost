...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  try {
    ...
    if (!raw.blocks) throw new Error("not editorjs");
    editorData = normalizeEditorJsDocument(raw);
  } catch {
...
function boot() {
  ...
  document.getElementById('save-draft-btn').addEventListener('click', () => savePost(0));
  document.getElementById('publish-btn').addEventListener('click', () => savePost(1));
...
async function savePost(publishedValue) {
  ...
  document.getElementById('hidden-published').value = String(publishedValue);
...
async function openPreview() {
  let data; try { data = await editor.save(); } catch { return; }
  const title = escapePreviewTitle(document.getElementById('post-title').value || 'Untitled');
  try {
...
</div>`, addAttribute(String(post.title ?? ""), "value"), post.published === 1 ? renderTemplate`<input id="post-slug" type="text"${addAttribute(String(post.slug ?? ""), "value")} placeholder="slug" readonly class="input input-bordered input-sm w-40 text-xs font-mono input-disabled opacity-70 cursor-not-allowed">` : renderTemplate`<div class="flex flex-col"> <input id="post-slug" type="text"${addAttribute(String(post.slug ?? ""), "value")} placeholder="slug" class="input input-bordered input-sm w-40 text-xs font-mono"> <p class="mt-1 text-xs text-base-content/50">Orijinal slug: ${String(post.slug ?? "")}</p> </div>`, post.published === 1 ? "Update" : "Publish", addAttribute(String(post.description ?? ""), "value"), post.published === 1 && renderTemplate`<p class="mt-1 text-xs text-warning">Slug degistirilemez.</p>`, defineScriptVars({ editorDataJson: editorData ? JSON.stringify(editorData) : "null" }));
