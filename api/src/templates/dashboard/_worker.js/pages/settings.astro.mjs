...
const $$Settings = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const siteConfigRows = await Astro2.locals.runtime.env.DB.prepare(
    `SELECT key, value FROM config
     WHERE key IN ('site_title', 'site_description', 'author_name', 'author_bio', 'blog_layout', 'share_twitter', 'share_linkedin', 'share_facebook', 'share_whatsapp', 'share_copy', 'site_lang')`
  ).all();
