...
/* empty css                                  */
import { c as createComponent, d as renderComponent, r as renderTemplate, b as createAstro, m as maybeRenderHead, a as addAttribute } from '../chunks/astro/server_C7eUu1_9.mjs';
import { $ as $$Base } from '../chunks/Base_BVvlZA7g.mjs';
import { $ as $$PostCard } from '../chunks/PostCard_NpzteZIA.mjs';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const featuredDate = featuredPost ? new Date(featuredPost.created_at).toLocaleDateString("tr-TR", {
    year: "numeric",
    month: "long",
    day: "numeric"
  }) : "";
  ...
  return renderTemplate`${renderComponent($$result, "Base", $$Base, { "config": config, "title": listTitle, "description": config.site_description, "canonicalPath": "/blog" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<h1 class="text-4xl font-bold mb-8">All Posts</h1> ${blogLayout === "grid" ? renderTemplate`<div class="grid grid-cols-1 gap-6 md:grid-cols-2"> ${posts.map((post) => renderTemplate`${renderComponent($$result2, "PostCard", $$PostCard, { "post": post })}`)} </div>` : blogLayout === "featured" ? renderTemplate`<div class="space-y-6"> ${featuredPost && renderTemplate`<article class="card border border-base-300 bg-base-100 shadow-sm transition hover:shadow-md"> ${featuredThumbnail && renderTemplate`<figure class="max-h-96 overflow-hidden"> <img${addAttribute(featuredThumbnail, "src")} alt="" class="h-full w-full object-cover"> </figure>`} <a${addAttribute(`/blog/${featuredPost.slug}`, "href")} class="card-body block"> <h2 class="mb-2 text-2xl font-bold hover:text-primary">${featuredPost.title}</h2> ${featuredDescription && renderTemplate`<p class="mb-4 text-base-content/80">${featuredDescription}</p>`} <time class="text-sm text-base-content/60">${featuredDate}</time> </a> </article>`} ${remainingPosts.length > 0 && renderTemplate`<div class="grid grid-cols-1 gap-6 md:grid-cols-2"> ${remainingPosts.map((post) => renderTemplate`${renderComponent($$result2, "PostCard", $$PostCard, { "post": post })}`)} </div>`} </div>` : renderTemplate`<div class="space-y-6"> ${posts.map((post) => renderTemplate`${renderComponent($$result2, "PostCard", $$PostCard, { "post": post })}`)} </div>`}` })}`;
